            ...
            <div class=" m-r-l  col">
                <div class="my-4 d-inline-block w-full ">
                    ...
                    @if (!isset($status) || $status->status_dispatch == 1)
                        ...
                        <a href="{{ route('status_dispatch', ['status_dispatch' => 2]) }}"
                            class="btn btn-secondary btn-sm text-uppercase">sim</a>
                    @elseif($status->status_dispatch == 2)
                        <a href="{{ route('status_dispatch', ['status_dispatch' => 1]) }}"
                            class="btn btn-secondary btn-sm text-uppercase">Não</a>
                        <a class="btn btn-success  text-uppercase">sim</a>
                    @endif

                    <button class="float-r m-r-10 btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#require_dispatch"> <i class="fa fa-paper-plane"></i> REQUERER DESPACHO</button>
                </div>
                                ...
                                @foreach ($dispatch as $dispatchs)
                                    @if ($dispatchs->status == 1)
                                        <tr class="table-success">
                                            ...
                                            <td scope="row">
                                                <a class="btn btn-secondary btn-sm" title="Chamar"><i
                                                        class="fa fa-clipboard-check"></i></a>
                                                <a href="{{ route('action_dispatch', ['id_dispatch' => $dispatchs->id, 'action' => 2]) }}"
                                                    class="btn btn-success btn-sm" title="Despachado"><i
                                                        class="fa fa-check"></i></a>
                                                <a class="btn btn-secondary btn-sm" title="Apagar"><i
                                ...
                                @foreach ($dispatch as $dispatchs)
                                    @if ($dispatchs->status == 0)
                                        <tr>
                                            ...
                                            <td scope="row">{{ $dispatchs->descripition }}</td>
                                            <td scope="row">
                                                <a href="{{ route('action_dispatch', ['id_dispatch' => $dispatchs->id, 'action' => 1]) }}"
                                                    class="btn btn-primary btn-sm" title="Chamar"><i
                                                        class="fa fa-clipboard-check"></i></a>
                                                <a href="{{ route('action_dispatch', ['id_dispatch' => $dispatchs->id, 'action' => 2]) }}"
                                                    class="btn btn-success btn-sm" title="Despachado"><i
                                                        class="fa fa-check"></i></a>
                                                <a href="{{ route('action_dispatch', ['id_dispatch' => $dispatchs->id, 'action' => 3]) }}"
                                                    class="btn btn-danger btn-sm" title="Apagar"><i
                                                        class="fa fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    @endif
            ...
            <div class="modal-content">
                ...
                <div class="c-b modal-body">
                    <form action="{{ route('require_dispatch') }}" method="POST">
                        ...
                        <div class="wrap-input">
                            <select required class="input" name="user" id="user">
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->user->id }}">{{ $user->user->rank->rankAbbreviation }}
                                        {{ $user->user->professionalName }}</option>
                                @endforeach
                            </select>
                        ...
                        <!--nome guerra-->
                        <textarea required class="textarea" maxlength="250" placeholder="Digite a mensagem..."
